...
func (b *Bucket) GetAll() map[interface{}][]byte {
	vm := make(map[interface{}][]byte)
	b.db.View(func(tx *bolt.Tx) error {
		bt := tx.Bucket(b.Name)
		if bt == nil {
			return fmt.Errorf("%s bucket does not exist", string(b.Name))
		}
...
// Len returns the item number in the bucket
