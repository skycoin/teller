...
func TestSKYNewStore(t *testing.T) {
	...
	err = s.AddSupportedCoin(CoinTypeSKY)
	require.NoError(t, err)

	err = s.db.View(func(tx *bolt.Tx) error {
		scanBktFullName := MustGetScanMetaBkt(CoinTypeSKY)
...
func TestSKYGetDepositAddresses(t *testing.T) {
	db, shutdown := testutil.PrepareDB(t)
	defer shutdown()
	...
	for _, a := range addrs {
		err := s.AddScanAddress(a, CoinTypeSKY)
		require.NoError(t, err)
	}

	as, err := s.GetScanAddresses(CoinTypeSKY)
	require.NoError(t, err)
...
func TestSKYAddDepositAddress(t *testing.T) {
	addrs := []string{
		"a1",
		"a2",
		"a3",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, a := range tc.addAddrs {
				if er := s.AddScanAddress(a, CoinTypeSKY); er != nil {
					err = er
				}
			}
...
func TestSKYPushDeposit(t *testing.T) {
	db, shutdown := testutil.PrepareDB(t)
	defer shutdown()
...
func TestSKYPutBktValue(t *testing.T) {
...
func TestSKYGetBktValue(t *testing.T) {
...
func TestSKYScanBlock(t *testing.T) {
	// TODO
}
